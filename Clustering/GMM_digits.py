...
n_components = np.arange(50, 210, 10)  # range of number of clusters
models = [GaussianMixture(n, covariance_type='full', random_state=0) for n
          in n_components]  # we define a model per number of clusters
...
gmm = GaussianMixture(n_components[np.argmin(aics)], covariance_type='full',
                                  random_state = 0)
gmm.fit(data)
